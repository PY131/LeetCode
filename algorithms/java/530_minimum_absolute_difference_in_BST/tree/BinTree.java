...
public class BinTree {
     ...
     *****************************************************/
    public TreeNode createTree(Integer[] data, int index, int n){
        TreeNode pNode = null;
        
        if(index < n && data[index] != null)  //数组中NULL表示该节点为空
        {
            pNode = new TreeNode(data[index].intValue());  //new mem space
            pNode.left = createTree( data, 2 * index + 1, n);  //loop through the b_tree using layer index, form 0 on
            pNode.right = createTree( data, 2 * index + 2, n);
        }
        return pNode;
    }
